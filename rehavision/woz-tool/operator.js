...
function renderPhrases() {
  PHRASE_GROUPS.forEach((group) => {
    ...
    group.phrases.forEach((phrase) => {
      ...
      btn.addEventListener("click", () => appendPhrase(phrase, group.category));
...
function renderLog() {
  ...
  if (log.length === 0) {
    ...
    td.colSpan = 6;
    td.textContent = "まだ記録がありません。応答を送信すると1行ずつ追加されます。";
    tr.appendChild(td);
  ...
  log.forEach((entry, index) => {
    ...
    tr.appendChild(textareaCell(entry.question, (v) => updateEntry(index, "question", v)));
    tr.appendChild(textareaCell(entry.answer, (v) => updateEntry(index, "answer", v)));
    tr.appendChild(categoryCell(entry.category, (v) => updateEntry(index, "category", v)));
...
function exportCsv() {
  ...
  const rows = log.map((e) => [e.id, e.time, e.question, e.answer, e.category]);
...
channel.addEventListener("message", (event) => {
  ...
  if (msg.type === "display-ready" || msg.type === "pong") setLinkStatus(true);
  if (msg.type === "display-closed") setLinkStatus(false);
});
...
els.thinkingBtn.addEventListener("click", () => channel.postMessage({ type: "thinking" }));
els.idleBtn.addEventListener("click", () => channel.postMessage({ type: "idle" }));
...
els.clearLogBtn.addEventListener("click", () => {
  ...
  if (!confirm(`記録済みの${log.length}件をすべて消去します。よろしいですか？`)) return;
  log = [];
  persist();
  renderLog();
});
